...
@app.route('/atualizar-inicio', methods=['POST', 'OPTIONS'])
def atualizar_inicio():
    ...
    try:
        ...
        novo_inicio = dados.get('inicio')

        if novo_inicio:
            global inicio, rota, custo
            if novo_inicio != inicio:
                inicio = novo_inicio
                rota, custo = vizinho_mais_proximo(grafo, inicio)
                return jsonify({
                    'message': f'Ponto de partida atualizado para {novo_inicio}.',
                    'rota': rota,
                    'custo': custo
                }), 200
            return jsonify({'message': 'O ponto de partida já está configurado como esse.'}), 200
        return jsonify({'error': 'Novo ponto de partida não fornecido!'}), 400
...
@app.route('/atualizar-tempo', methods=['POST', 'OPTIONS'])
def atualizar_tempo_espera():
    ...
    novo_tempo = dados.get('tempo')
    
    if brinquedo in brinquedos:
        # Atualizar o tempo de espera
        brinquedos[brinquedo]['tempo_espera'] = novo_tempo
        
        # Recriar o grafo completo
        global grafo
        grafo = criar_grafo_completo(brinquedos)
        
        # Recalcular a rota
        global rota, custo
        rota, custo = rota, custo = vizinho_mais_proximo(grafo, inicio)
        
        return jsonify({
            "mensagem": "Tempo atualizado e grafo recalculado com sucesso!",
            "rota": rota,
            "custo": custo
        })
    return jsonify({"erro": "Brinquedo não encontrado"}), 400
@app.route('/reset', methods=['POST'])
def resetar_dados():
    global brinquedos, grafo, rota, custo

    # Dados iniciais dos brinquedos
    brinquedos_iniciais = {
        'Insano': {'tempo_espera': 20, 'distancias': {'Kalafrio': 100, 'Vaikuntudo': 150, 'Maremoto': 200, 'Arrepius': 180, 'Ramubrinká': 170, 'Tobomusik': 90, 'Atlantis': 120, 'Aqua Show': 180, 'Moréia Negra': 150, 'Ilha do Tesouro': 100}},
        'Kalafrio': {'tempo_espera': 10, 'distancias': {'Insano': 100, 'Vaikuntudo': 120, 'Arrepius': 180, 'Ramubrinká': 170, 'Tobomusik': 110, 'Atlantis': 140, 'Aqua Show': 160, 'Moréia Negra': 180, 'Ilha do Tesouro': 130}},
        'Vaikuntudo': {'tempo_espera': 15, 'distancias': {'Insano': 150, 'Kalafrio': 120, 'Maremoto': 90, 'Arrepius': 200, 'Ramubrinká': 150, 'Tobomusik': 100, 'Atlantis': 130, 'Aqua Show': 160, 'Moréia Negra': 140, 'Ilha do Tesouro': 180}},
    ...
    brinquedos = brinquedos_iniciais.copy()
    coordenadas = coordenadas_iniciais.copy()
    ...
    return jsonify({"mensagem": "Dados resetados com sucesso!", "rota": rota, "custo": custo})
